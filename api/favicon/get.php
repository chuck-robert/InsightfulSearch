...
if($expire == 0) {
	...
	if (preg_match('/xnAppId/i',$_GET['url'])) {
		//读取JSON数据;
		$JsonAppData = file_get_contents("./../../data/json/appList.json");
		//数组解码;
		$arr = json_decode($JsonAppData, true);
		//判断并过滤数组
		foreach ($arr["appDataList"] AS $k => $v) {
			if (array_key_exists("icon", $arr["appDataList"][$k])) {
				// echo str_replace(')','',str_replace('xnAppId(','',$_GET['url']));
				if($v["id"] == str_replace(')','',str_replace('xnAppId(','',$_GET['url']))) {
					// echo file_get_contents($v['icon']);
				}
			}
		}
	} else {
	...
	if( md5($content) == $defaultMD5 ) {
		$expire = 43200;
		//如果返回默认图标，设置过期时间为12小时。Cache::get 方法中需同时修改
	}
